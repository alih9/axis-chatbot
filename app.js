...
} = require("./controller/SocketUserController");
...
setTimeout(() => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("add_active_user", async (data) => {
      console.log(
        `----------------->Add active User ${data.email} and ${data.is_active}`
      );
      //gets the room user and the message sent
      const active_user = await add_active_users(
        socket.id,
        data.email,
        socket,
        data.is_active
    ...
    socket.on("joinRoom", async ({ username, roomname }) => {
      //* create user
      console.log(
        `----------------->User ${username} joining the room ${roomname}`
      );
    ...
    socket.on("get_active_customers", () => {
      var user_arr = [];
      console.log(
        "---------------GET ACTIVE CUSTOMER EVENT INVOKED----------------"
      );
      get_active_customers()
        .then(async (data) => {
          for (var i = 0; i < data.length; i++) {
            console.log("FOR LOOP RUNNING", data[i]);
            await get_room(data[i].email).then((room) => {
              if (room != null) {
    ...
    socket.on("deactivate_room", (room) => {
      ...
      console.log(
        "deactivate_roomdeactivate_roomdeactivate_roomdeactivate_roomdeactivate_room"
      );
