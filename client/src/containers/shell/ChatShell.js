  ...
  useEffect(() => {
    ...
    if (Object.keys(selectedConversation).length != 0) {
      ...
      if (selectedConversationId != "") {
        ...
        console.log(
          "John has left this conversation ",
        ...
        socket.emit("room_activation_status", active_chats);
        console.log(
          "John has joined this conversation ",
          selectedConversation.id
        );
        setselectedConversationId(selectedConversation.id);
      }
...
const mapDispatchToProps = (dispatch) => ({
  ...
  updateConversation: (conversationId, user, time, last_message) => {
    dispatch(updateConversation(conversationId, user, time, last_message));
  },
  deletedAddedConversation: (conversationId) => {
    dispatch(deletedAddedConversation(conversationId));
  },
  deleteSelectedConvsersation: () => {
    dispatch(deleteSelectedConvsersation());
  },
  onDeleteConversation: () => {
  ...
  messageDeleteDetail: (room_id) => {
    dispatch(messageDeleteDetail(room_id));
  },
  loadConversations: (type) => {
